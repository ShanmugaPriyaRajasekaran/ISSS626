...
We determine the optimal adaptive bandwidth for performing Geographically Weighted Regression (GWR). Specifically, we are interested in finding the best bandwidth to use for summarizing the spatial variation in GDP per capita (GDPPC) across the Hunan region.
...
The abbreviation in the output refers to:
...
We will observe similar trend using Local Spearman Coefficient
