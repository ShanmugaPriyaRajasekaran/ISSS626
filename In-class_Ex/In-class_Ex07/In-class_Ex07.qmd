...
comparison
```
Let us see the distribution of the R2 in the Bishan Planning Area
The Local R² values in Bishan show a moderately strong to strong model performance across the planning area, with values ranging from 0.651 to 0.745. This indicates that the GWR model explains between 65.1% to 74.5% of the variation in condominium prices across different locations within Bishan.

**Key Observations:**
**Consistently Good Model Fit:** The entire planning area shows R² values above 0.65, suggesting the hedonic pricing model performs reasonably well throughout Bishan without any particularly weak spots.

**Spatial Gradient Pattern:** The R² values appear to follow a spatial pattern rather than being randomly distributed. There are clear clusters of higher and lower explanatory power, indicating that the model captures local relationships more effectively in certain sub-regions.

**No Extremely Poor Performing Areas:** The absence of low R² values (below 0.6) suggests that the selected variables (property size, age, proximity to amenities, etc.) are generally relevant for explaining price variations across all of Bishan.

**Moderate Variation:** The range of approximately 0.1 (from 0.651 to 0.745) indicates that while there are differences in model performance across the area, these differences are not extreme. The model maintains relatively stable explanatory power throughout Bishan.

This distribution pattern suggests that Bishan's condominium market follows relatively predictable patterns that are well-captured by the standard hedonic pricing variables, with some subtle local variations in how these factors influence prices across different neighborhoods within the planning area.
