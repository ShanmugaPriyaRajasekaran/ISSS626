...
Summary just to check if there are any missing data
...
Append all the population figures to geometric figures
```{r}
village <- village %>%
  left_join(pop2019,
            by = join_by(
...
There are 5 missing values , we need to tidy it up
```{r}
summary(village)
```
Removing the 5 missing records,
```{r}
village <- village %>%
  filter(!is.na(P_CNT)) 
```
