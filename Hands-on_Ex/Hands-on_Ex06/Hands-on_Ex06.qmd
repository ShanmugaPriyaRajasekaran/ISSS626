...
In this hands-on exercise, we will learn to delineate homogeneous regions using hierarchical and spatially constrained clustering techniques on geographically referenced multivariate data.
...
::: insightbox
**Interpretation**
We have 6 fields of Multipolygon Geometry type and WGS 84 is the Geodetic CRS.
:::
...
p <- ggplot(data = ict_derived, aes(x = RADIO)) +
  geom_histogram(bins = 20, color = "black", fill = "#e8d9b8") +
  labs(
  ...
  theme_minimal() +
  theme(
    plot.background = element_rect(fill = "#fffbe8", color = NA),
    panel.background = element_rect(fill = "#fffbe8", color = NA)
  )

# Convert to plotly and set background
p_plotly <- ggplotly(p) %>% 
  layout(plot_bgcolor = '#fffbe8',
         paper_bgcolor = '#fffbe8')
...
plot_ly(data = ict_derived,
        ...
        type = 'box',
        fillcolor = '#e8d9b8',
        line = list(color = '#bfa87d'),
        marker = list(color = '#bfa87d'),
        notched = TRUE
       ) %>%
  layout(
    title = "Distribution of Radio Ownership",
...
radio <- ggplot(data=ict_derived,
  ...
  geom_histogram(bins=20,
                 color="black",
                 fill="#e8d9b8") +
  theme(plot.background = element_rect(fill = "#fffbe8"),
        panel.background = element_rect(fill = "#fffbe8"))
...
The code chunk below performs hierarchical cluster analysis using ward.D method. The hierarchical clustering output is stored in an object of class **hclust** which describes the tree produced by the clustering process.We can then plot the tree by using *plot()* of R Graphics.


```{r plot_dendrogram, fig.width=10, fig.height=6}
par(bg = "#fffbe8")
# Create and plot the hierarchical clustering
...
# Set background color for base R plot
par(bg = "#fffbe8")
...
We will then join back the group list with *shan_sf* polygon feature data frame by using the code below.We can now plot the map of the newly delineated spatially constrained clusters.
