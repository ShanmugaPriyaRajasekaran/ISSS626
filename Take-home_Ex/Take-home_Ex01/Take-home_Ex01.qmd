...
## Select the 4 SSIC divisions (our study)
...
## Build map-ready points → clip → tag subzones
...
saveRDS(pts_ready, "data/rds/acra_points_2024_2025.rds")

```

::: insightbox
Insight: Rows are now SVY21 points in metres, safely inside the MP2019 window, and each record carries SUBZONE/PLANNING AREA labels. This is the backbone for dot maps, KDE, and subzone scoreboards.
:::


## QA: coverage, CRS, sector counts
...
# CRS must be 3414
sf::st_crs(pts_ready)$epsg

# Any missing labels?
sum(is.na(pts_ready$SUBZONE_N))
sum(is.na(pts_ready$PLN_AREA_N))

# Only our four divisions?
pts_ready |>
  sf::st_drop_geometry() |>
...
::: insightbox
Insight: inside_share should be ~1 and EPSG should read 3414. Near-zero missing subzone labels means your join worked and you can trust subzone aggregations.
:::

# Starter EDA: monthly inflow & top planning areas
...
::: insightbox
Insight: This shows when each sector peaked and where concentrations are strongest (planning areas). Perfect for captions and to prioritise follow-up maps.
:::

# Choropleth: Counts by Planning Area (all four SSIC divisions)
...
# Choropleth: Counts by Planning Area × Sector (faceted)
...
# Choropleth: Counts by Subzone × Sector (finer grain)
...
# First-Order Setup, Build Window & ppp Converters
...
```{r}
# Window (country outline) from your filtered subzones — in SVY21 metres
...
::: insightbox
Insight
All spatial analyses now run in metres (SVY21). This ensures distance-based quantities (nearest neighbour, KDE bandwidths) are numerically meaningful and edge-corrected by the true study window
:::

## Clark–Evans Nearest-Neighbour Test (per sector)
...
```{r}
# Clark–Evans for each sector — asymptotic + Monte Carlo (with small-n guards)
library(dplyr); library(purrr); library(spatstat.explore)

ce_one <- function(df, sector_label, nsim = 999L) {
  sub <- df %>% dplyr::filter(sector == sector_label)
  if (nrow(sub) < 10) {
    return(tibble(
      sector = sector_label, n = nrow(sub),
  ...
  z_mc <- spatstat.explore::clarkevans.test(
    X, correction = "none", clipregion = W,
    alternative = "clustered", method = "MonteCarlo", nsim = nsim
  )
...
set.seed(42)
ce_results <- map_dfr(sectors4, ~ ce_one(biz_pts_use, .x, nsim = 999L))
ce_results
...
# Spatial KDE — Reusable Helpers (one call per sector)
...
::: insightbox
Insight
We cache images and bandwidths so you don’t recompute each knit. The 300–800 m clamp stabilises small-n sectors and avoids over-smooth blobs for large-n sectors.
:::

## KDE — F&B (56) baseline + breaks
...
::: insightbox
Insight
Expect clusters along leisure belts (parks, waterfronts) and lifestyle hubs, often overlapping with F&B but with slightly broader halos around large complexes and event nodes.
:::

## KDE — Air Transport (51)
```{r}
...
::: insightbox
Insight
Intensity should skew eastwards around airport/logistics corridors; patterns are estate-anchored and more punctual than consumer sectors.
:::
...
::: insightbox
Insight
Siting is regulatory/infrastructure-led. Expect few, stable anchors near treatment plants/reservoirs or utility corridors; KDE may appear sparse by design.
:::

# Spatio-Temporal KDE (Monthly) — Helpers
...
tau_days <- 60  # temporal bandwidth (≈ 2 months std dev)

```

::: insightbox
Insight
ST-KDE reveals when hotspots flare or fade. Using sector-specific spatial bandwidths and a common temporal bandwidth keeps maps comparable without over-smoothing small sectors.
:::

# 2nd-Order Spatio-Temporal Analysis (Ripley K/L, ST-K)
...
::: insightbox
Insight: We’re ready to measure where–when interactions across realistic city scales: 50–4000 m in space, 1 week–6 months in time.
:::

## Normalising stkhat() shapes (defensive helper)
...
# Spatio-temporal K with time-permutation envelopes (SG-wide)
...
compute_st_env <- function(X_xyz, s_vals, u_vals, poly, nsim = 199) {
  ...
  list(K_obs = K_obs, K_lo = lo, K_hi = hi, K_med = med, s = s_vals, u = u_vals)
}
...
for (sec in sectors4) {
  ...
  key <- paste0("stk_env_SG_", gsub("\\W","_",sec),
                "_ns", length(r_vals), "_nu", length(u_vals),
                "_nsim", nsim, ".rds")
  cache <- file.path("data/rds", key)

  force_recompute <- TRUE  # set FALSE to use cache
...
## Heatmap visualiser for ST-K excess
...
if (!is.null(st_sg[["Water (36)"]]))           print(plot_st_heat_tile(st_sg[["Water (36)"]],           "Water (36) — ST-K excess"))
...
## Subzone-scale second-order (Ripley’s L within a subzone)
